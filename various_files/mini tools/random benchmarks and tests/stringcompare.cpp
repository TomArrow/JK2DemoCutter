...
void compareStrcmp(){
    ...
    for(int r=0;r<100000;r++){
        for(int i=0;i<stringcount;i++){
            matches += !strcmp("Red hit the capturelimit.",strings[i]);
...
void compareStrcasecmp(){
    ...
    for(int r=0;r<100000;r++){
        for(int i=0;i<stringcount;i++){
            matches += !strcasecmp("Red hit the capturelimit.",strings[i]);
            //matches += !memcmp("Red hit the capturelimit.",strings[i],26);
        }
    }
    std::cout<<matches<<" matches\n";
}
constexpr auto theStrng = "Red hit the capturelimit."_cs;
void compareMine(){
    int matches = 0;
    for(int r=0;r<100000;r++){
        for(int i=0;i<stringcount;i++){
            
            matches += theStrng.unsafematch(strings[i],stringLengths[i]);
        }
    }
    std::cout<<matches<<" matches\n";
}
